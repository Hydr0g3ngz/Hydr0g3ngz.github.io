...
function updateStatus() {
  ...
  const dirty = JSON.stringify(d.data) !== d.saved;
...
function selectDocument(id) {
  lastEdit = { path: '', at: 0 };
  try { $('#preview').contentDocument?.querySelector('[contenteditable="true"]')?.blur(); } catch {}
...
function renderField(input, path) {
  ...
  if (field.list) {
    ...
    if (field.type === 'boolean') {
      ...
      if (field.type === 'rich-text') wrapper.append(node('div', { class: 'markdown-toolbar' }, [['Bold', '**', '**'], ['Italic', '*', '*'], ['Heading', '## ', ''], ['Link', '[', '](https://example.com)'], ['Quote', '> ', '']].map(([title, before, after]) => button(title, () => { const start = control.selectionStart; const end = control.selectionEnd; control.setRangeText(before + control.value.slice(start, end) + after, start, end, 'select'); control.dispatchEvent(new Event('input')); control.focus(); }, { class: 'mini-button' }))));
      if (field.type === 'image') {
...
function openModal(title, content) { $('#modal-title').textContent = title; $('#modal-content').replaceChildren(content); if (!$('#modal').open) $('#modal').showModal(); }
...
window.addEventListener('message', event => {
  ...
  if (message.type === 'edit' && Array.isArray(message.path) && message.path.every(key => typeof key === 'string' && !['__proto__', 'constructor', 'prototype'].includes(key))) mutate(data => setAt(data, message.path, message.value), { render: true, path: message.path });
...
function newPage() {
  ...
  const kind = node('select', { name: 'kind' }, [node('option', { value: 'page', text: 'Page — build with sections' }), node('option', { value: 'note', text: 'Note — write with Markdown' })]); const route = node('p', { class: 'notice', text: 'New pages start as drafts. Their URL can include folders.' }); let slugTouched = false;
...
$('#interact').onclick = () => { state.mode = state.mode === 'edit' ? 'interact' : 'edit'; $('#interact').textContent = state.mode === 'edit' ? '↖ Edit mode' : '▷ Interact'; $('#interact').setAttribute('aria-pressed', state.mode === 'interact'); if (state.mode === 'edit') schedulePreview(0); bridge({ type: 'mode', mode: state.mode }); };
document.addEventListener('keydown', event => { if (!(event.ctrlKey || event.metaKey)) return; if (event.key.toLowerCase() === 's') { event.preventDefault(); save(); } else if (event.key.toLowerCase() === 'z' && !['INPUT', 'TEXTAREA'].includes(event.target.tagName)) { event.preventDefault(); historyStep(event.shiftKey ? 'redo' : 'undo'); } });
window.addEventListener('beforeunload', event => { if ([...state.drafts.values()].some(d => JSON.stringify(d.data) !== d.saved)) event.preventDefault(); });
